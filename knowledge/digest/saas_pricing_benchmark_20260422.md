...
1. **per-seat課金の脱却**：2024年64% → 2025年57%に減少。iPaaS系（Zapier／Make／n8n／Yoom／IFTTT）は全社「ユーザー無制限＋利用量課金」に移行済み。
2. **Free無期限プランが標準装備**：調査34サービス中28がFree提供。「14日トライアルのみ」は業界と逆行リスク。
3. **「何人から」下限表現は少数派**：調査34プラン中わずか4件（12%）。多数派は「1名から／per-seat」または「無制限／flat」。

**Kawaru料金案の位置づけ**：PERSONAL ¥9,000（$60相当）はグローバル上位26%。TEAM ¥14,900／人（$100相当）はper-seat系で上位10%。**価格帯は高めに振れており、per-seat × 下限設定 × 高単価という設計は業界トレンドの真逆に立っている**。
...
| パターン | 件数 | 割合 | 代表例 |
|----------|------|------|--------|
| **無制限（flat）** | 11 | 32% | Zapier全プラン、Make全プラン、Yoomサクセス、IFTTT、n8n Cloud |
| **「1名あたり」per-seat** | 14 | 41% | Notion／Slack／Figma／HubSpot／Asana／Salesforce／Zoom／Perplexity |
| **「〜名まで」上限** | 5 | 15% | Yoom ミニ20名・チーム100名、Dify Pro 3・Team 50 |
| **「〜名から」下限** | 4 | 12% | ChatGPT Business（2名〜）、Claude Team（5名〜）、Dropbox（3名〜）、**Kawaru TEAM（5名〜）** |

> **観察**：「〜名から」下限表現は少数派（12%）。採用しているChatGPT Business・Claude Teamはいずれも大手LLM本家で、**「セールス主導で取りに行く層」を明示的に絞っている**商品。Dropboxの「3名〜」はビジネスプランの伝統的設計。Kawaruが並ぶのはこの4社のみ。
>
...
| 課金軸 | 件数 | 代表例 | 特徴 |
|--------|------|--------|------|
| per-seat pure | 11 | 汎用SaaS全般、Perplexity | 予測可能性高い、AIと相性悪い |
| usage-based | 7 | Zapier／Make／n8n／Yoom／Dify／IFTTT／Adobe | 価値と連動、変動コストあり |
| ハイブリッド（seat＋credit） | 3 | Cursor Business、ChatGPT Business、Claude Team | 新世代の主流 |
| 成果課金（outcome） | 1 | Intercom Fin（$0.99／解決） | ROI透明だが逆説的 |
| flat-rate無制限 | 2 | SHIFT AI、Cursor個人 | シンプル訴求 |

**トレンドデータ**（Gartner／IDC／a16z／Bessemer 複数出典）：
- per-seat採用率：64%（2024）→ 57%（2025）
- usage-based採用率：45%（2021）→ 61%（2025）
- seat-based pure採用率：21% → 15%（1年で-6pt）
- **2026年までに70%の企業がper-seatよりusage-basedを選好**（Gartner予測）
- **2028年までに70%のSaaSベンダーが純粋per-seatから脱却**（IDC予測）
...
- TEAM ¥14,900／人 ≒ $100 → HubSpot Pro／Salesforce Pro相当、per-seat系では上位10%
...
| Gemini×Workspace | Starter $9.20／Std $18.40／Plus $22 | per-seat |

#### カテゴリB：iPaaS・自動化（Kawaruに最も近い）
| サービス | 料金構造 | 人数扱い |
|---------|---------|---------|
| Zapier | Free／Pro $20／Team $69／Ent要相談 | **全プラン無制限** |
| Make | Free／Core $9／Pro $16／Teams $29／Ent要相談 | **全プラン無制限** |
| n8n Cloud | Starter €24／Pro €60／Business €667／Ent要相談 | 実質無制限 |
| Dify Cloud | Sandbox $0／Pro $59／Team $159／Ent要相談 | **1名／3名まで／50名まで** |
| **Yoom新体系** | Free／Personal ¥2,400／Mini ¥16,000／Team ¥40,000／Success ¥80,000 | 1／1／**20まで**／**100まで**／**無制限** |
| IFTTT | Free／Pro $3.49／Pro+ $14.99 | 1名 |

#### カテゴリC：AI研修系（Kawaru直接競合）
| サービス | 料金構造 | 備考 |
|---------|---------|------|
| SHIFT AI | ¥21,780／月（税込）／一括¥547,800 | 個人向け flat |
...
#### カテゴリD：AIネイティブ
Cursor Hobby $0／Pro $20／Pro+ $60／Ultra $200／Business $40・Ent要相談（料金＝AIクレジット額）／GitHub Copilot Free／Pro $10／Pro+ $39／Business・Ent要相談／Perplexity Free／Pro $20／Max $200／Ent Pro $40・Max $325／Notion AI統合（$20／user）／Granola Free／Individual $18／Business $14・Ent $35／Otter Pro $8.33／Business $19.99／Ent要相談

#### カテゴリE：汎用SaaS（比較基準）
Notion Plus $10／Business $20／Ent $25-30（Guest無料）／Slack Pro $7.25／Biz+ $12.50／Grid要相談／Figma Pro $12／Org $45／Ent $90（4種seat、View無料）／HubSpot Starter $15／Pro $100／Ent $150（View-Only無料）／Asana Starter $10.99／Adv $24.99／Ent $35／Ent+ $45／Zoom Pro $13.33／Business $18.33／Biz+ $29／Salesforce Starter $25／Pro $100／Ent $175／Unlimited $350／Agentforce1 $550／Dropbox Standard $18／Advanced $30（**3名〜**）／MS365 Basic $6／Standard $12.50／Premium $22
...
**業界通説**：無料 → 個人 → チーム → エンタープライズ の4段構造がPLGの定石。境界線は「顧客価値が非連続に跳ね上がる点」に引く。
...
| 表現 | 発信メッセージ | 商品観 |
|------|---------------|--------|
| 「〜名まで」 | 「ここまでは面倒見る」 | パッケージ商品 |
| 「〜名から」 | 「ここから取引開始」 | 法人営業商品 |
| 「1名あたり」 | 「人数に比例して課金」 | 席売りツール |
| 「無制限」 | 「組織価値を提供」 | プラットフォーム |

「〜名から」下限表現は、**法人営業商品の発信**。LP主導の自己完結型購買導線とは構造的に相性が悪い。日本企業の稟議文化（小さく始めて効果検証→拡大）とも齟齬がある。
...
### 論点3：per-seat vs usage-based vs flat-rate の使い分け原理

課金軸は「プロダクトの価値創出メカニズムと一致させよ」という第一原理に帰着する。

| 課金軸 | 適合条件 | Kawaruへの適合度 |
|--------|---------|-----------------|
| per-seat | 人が操作することで価値が出る／予測可能性重視 | 部分適合 |
| usage-based | 利用量と価値が線形／コスト変動を転嫁したい | 高適合（ワークフロー実行数） |
| outcome-based | 成果が明確に測定可能／性能に自信 | 中適合（削減工数） |
| flat-rate | COGSが急速に低下／単純さで差別化 | 中適合（シンプルさで日本市場に刺さる） |

2026年のコンセンサスは**ハイブリッド**（基本サブスク＋従量／成果アドオン）。純粋単一モデル比+21%の成長率（Monetizely 2026）。Yoomの2026年5月改定（ライセンス廃止＋タスク課金強化）も同じ構造転換。
...
#### 1位：per-seat × 高単価のAI時代リスク
業界全体がper-seat脱却に動く中、2026年ローンチの新規SaaSがper-seatで¥14,900／人は**構造的に逆行**。Yoomが2026年5月にper-seat廃止へ動いた事例を見れば、Kawaruも遠からず同じ修正を迫られる可能性が高い。**既存契約との整合性問題が後から発生する**リスクを今から認識しておくべき。
...
|---------|-------------|----------|---------|
| Zapier Team | 約¥10,400（$69、ユーザー無制限） | 約¥2,080 | **1/7** |
...
### 事例1：Zapierのタスク課金（per-seat完全廃止）
Team $69／月でユーザー無制限。タスク実行数＝価値提供量と連動。社内展開の摩擦ゼロが最大の武器。

### 事例2：Yoom 2026/5改定
ライセンス課金廃止（ミニ3→20、チーム10→100、サクセス20→無制限）、基本料金+67%。Free→ミニ¥9,600の断崖をPersonal¥2,400で埋めた（PLG転換率改善狙い）。

### 事例3：Intercom Fin の成果課金
$0.99／resolution。AIが解決したときだけ課金＝ROI透明。$1M→$100M+ ARR急伸。ただし「成功するほど請求が増える」パラドックス。

### 事例4：Adobe Generative Credits
AIコンピュート費用を既存SaaS料金に内包、月次付与＋繰越なし（使用圧力＝アップセル機会）。

### 事例5：Cursor「プラン料金＝AIクレジット額」
Pro $20＝月$20分のプレミアムモデル利用枠。Auto modeは無制限。支払額とAIコストが1:1。透明性で信頼獲得。

### 事例6：Claude Team Standard/Premium混在
最小5 seats、Standard $20＋Premium $100を混在可能。パワーユーザーとカジュアルユーザーを1契約で運用できる柔軟性。

### 事例7：HubSpot View-Only / Figma View Seat
閲覧専用は無料・無制限。情報共有の摩擦を下げる＝導入拡大の潤滑油。Kawaruでも「閲覧専用メンバー」の概念導入で組織内展開の摩擦を下げる余地あり。

### 事例8：Dropboxの「3名から」
per-seat下限の数少ない採用例。ただしDropboxは「ファイル共有が本質＝3名以上で意味を持つ」という機能的理由があり、下限設定の正当性がプロダクト側に内在している。Kawaruが下限を設けるなら同等の機能的正当性の説明が必要。
...
これらの答えが、下記を一意に規定する：
- 課金軸（per-seat／usage／flat／hybrid）
- 人数表現（上限／下限／per-seat／無制限）
- 階段の段数と境界
- 無料プランの設計
- 改定前提の有無
...
- [[Yoom_pricing_analysis_20260416]] — Yoom 2026年5月料金改定の背景分析（per-seat脱却トレンドの起点）
