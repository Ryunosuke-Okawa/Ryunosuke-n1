...
ty += 0.70
bk(slide, Mv, ty, CW, 0.55,
   'Antigravity×Claude CodeでLPを自作する行動力がある。正しいガイドがあれば加速できるポテンシャル。', 13,
...
branches = [
    ...
    ('B. AI活用の\n知識・スキル不足', [
        ...
        ('Antigravity×Claude CodeでLP制作は80%止まり', True),
        ('適切なツール選定ができない', False),
    ]),
    ('C. FC展開の\n基盤が未整備', [
        ('FC加盟店ゼロ、募集段階', True),
...
examples = [
    ('① LP制作・更新', 'Antigravity × Claude Code',
     '外注10〜50万円→自分で数時間。\nA/Bテストも即座に回せる。'),
    ('② SNS投稿文の自動生成', 'Dify + AI',
     '写真を入れるだけで\nInstagram投稿文が自動生成。'),
    ('③ 口コミ返信の自動下書き', 'n8n + AI',
...
p1_months = [
    ('1ヶ月目', 'AI基礎×LP制作入門', 'Antigravity等の操作を習得。簡易LPを1本制作'),
...
# =============================================
# POST-PROCESS: Remove all shadows from shapes
# =============================================
        ...
        if spPr is None:
            ...
            for child in sp:
                if child.tag.endswith('}spPr'):
                    ...
                    break
        if spPr is not None:
            # Remove existing effect lists (shadows)
            for eff in spPr.findall(qn('a:effectLst')):
                spPr.remove(eff)
            # Add empty effect list = explicitly no effects
            etree.SubElement(spPr, qn('a:effectLst'))

print('Shadows removed from all shapes.')
