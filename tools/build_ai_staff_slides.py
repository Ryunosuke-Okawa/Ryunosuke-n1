...
""")
    ...
    def s10_to_results(self):
        ...
        self._block(s, 12.0, 7.1, 1.1, 0.3, "8 / 13", 11, color=C['gray'], align=PP_ALIGN.RIGHT)
...
""")
    ...
    def s12_now_future(self):
        ...
        self._block(s, 0.5, 6.0, 12.33, 0.6,
                    ...
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        self._block(s, 12.0, 7.1, 1.1, 0.3, "10 / 13", 11,
                    color=C['gray'], align=PP_ALIGN.RIGHT)
...
""")
    ...
    def _roadmap_table(self, slide, rows, y_start=1.5, h_header=0.75, h_row=1.55,
                       size_month=14, size_level=20, size_task=14, size_reason=13):
        """ロードマップテーブル描画（共通）"""
        y = y_start
        # ヘッダー
        self._block(slide, 0.5, y, 1.5, h_header, '月', 16, bold=True,
                    color=C['white'], bg=C['navy'],
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        self._block(slide, 2.05, y, 1.4, h_header, '達成水準', 16, bold=True,
                    color=C['white'], bg=C['navy'],
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        self._block(slide, 3.50, y, 5.5, h_header, '具体タスク', 16, bold=True,
                    color=C['white'], bg=C['navy'],
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        self._block(slide, 9.05, y, 3.78, h_header, 'この時期に重点を置く理由', 16, bold=True,
                    color=C['white'], bg=C['navy'],
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        y += h_header + 0.1
        ...
        for month, level, task, reason, emp in rows:
            ...
            self._block(slide, 0.5, y, 1.5, h_row, month, size_month, bold=emp, color=fc_l, bg=bg_l,
                        align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
            self._block(slide, 2.05, y, 1.4, h_row, level, size_level, bold=True, color=fc_l, bg=bg_l,
                        align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
            self._block(slide, 3.50, y, 5.5, h_row, task, size_task, color=fc_r, bg=bg_r,
                        align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.MIDDLE)
            self._block(slide, 9.05, y, 3.78, h_row, reason, size_reason, color=fc_r, bg=bg_r,
                        align=PP_ALIGN.LEFT, anchor=MSO_ANCHOR.MIDDLE)
            y += h_row + 0.1
        return y

    def s13_roadmap_a(self):
        """ロードマップ前半（4月〜6月末）"""
        s = self._slide()
        self._bg(s)
        self._title(s, '9月末までのロードマップ ① 6月末まで', 11)
        ...
        rows = [
            ...
            ('5月', '1.6人分',
             ...
             'マーケ・営業の比重が大きい時期。\nAI社員を育てながら走らせる', False),
            ('6月末\n(中間)', '2人分',
             '8EXPO 出展 (6/3-4)\nローンチ後のフィードバック反映\n5人連携テスト',
             'デリバリー内容が変化する時期。\n顧客の声を聞きながらブラッシュアップ', True),
        ]
        self._roadmap_table(s, rows, y_start=1.5, h_header=0.7, h_row=1.55,
                            size_month=14, size_level=20, size_task=13, size_reason=13)

        self._block(s, 0.5, 6.85, 12.33, 0.45,
                    '5月の本番（ローンチ・展示会）が山場、6月末に2人分まで持っていく',
                    13, bold=True, color=C['kawaru'],
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)

        self._notes(s, """【ロードマップ前半・15秒】
9月末までのロードマップ前半、6月末までです。
4月の現在は、5月のβ版ローンチと6月の8EXPO展示会に向けた制作物作成が中心。
5月はローンチ実施と展示会の戦略・戦術設計にシフト。マーケと営業の比重が大きい時期です。
6月末で2人分。8EXPO出展とローンチ後のフィードバック反映、5人の連携テスト。デリバリー内容が変化する時期で、顧客の声を聞きながらブラッシュアップします。
""")

    def s13_roadmap_b(self):
        """ロードマップ後半（7-8月〜9月末）"""
        s = self._slide()
        self._bg(s)
        self._title(s, '9月末までのロードマップ ② 9月末まで', 12)
        ...
        rows = [
            ...
            ('9月末\n(最終)', '5人分',
             '5人フル稼働\nプロダクト × デリバリー 両輪完成',
             'Agent Teams で5人協働を実装し、\n事業を動かす完成形へ', True),
        ]
        # 2行なので大きめに
        self._roadmap_table(s, rows, y_start=1.5, h_header=0.8, h_row=2.3,
                            size_month=18, size_level=26, size_task=16, size_reason=15)

        self._block(s, 0.5, 6.85, 12.33, 0.45,
                    '7-8月で事務・法務AIを追加、9月末に5人完成',
                    13, bold=True, color=C['kawaru'],
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)

        self._notes(s, """【ロードマップ後半・15秒】
ロードマップ後半、9月末までです。
7-8月は3〜4人分。β版経由の新規案件対応とプロダクト改善ループ、事務・法務AIを追加します。顧客が増えて事務処理も増える時期です。
...
""")
    ...
    def build(self, output):
        ...
        self.s13_roadmap_a()    # 11枚目：ロードマップ前半（6月末まで）
        self.s13_roadmap_b()    # 12枚目：ロードマップ後半（9月末まで）
        self.s14_close()        # 13枚目：締め
        self.prs.save(output)
        print(f"Saved: {output}")


if __name__ == '__main__':
    output_path = '/Users/kyouyuu/claude/strategy/AI社員5人_発表_20260428.pptx'
