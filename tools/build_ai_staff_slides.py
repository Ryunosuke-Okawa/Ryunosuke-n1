...
""")
    ...
    def s02_premise(self):
        ...
        self._block(s, 0.5, 2.7, 12.33, 1.2, 'コストの話がしにくい',
...
最初に前提を共有します。
Kawaru事業部はコストの話がしにくい事業部です。
既存事業のように固定業務がない。業務が毎月変わるので、そもそも削減対象が見えない。
...
""")
    ...
    def s04_theme(self):
        """今回のテーマ：9月末までにAI社員5人を作る"""
        s = self._slide()
        self._bg(s)
        self._title(s, '今回のテーマ', 4)

        # サブ
        self._block(s, 0.5, 1.5, 12.33, 0.6, '今回のハッカソンで取り組むこと', 16,
                    color=C['gray'], align=PP_ALIGN.CENTER, bold=True)

        # メイン
        self._block(s, 0.5, 2.3, 12.33, 1.0, '9月末までに、',
                    32, color=C['navy'],
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        self._block(s, 0.5, 3.3, 12.33, 1.6, 'AI社員 5人 を作る',
                    56, bold=True, color=C['kawaru'],
                    align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
        ...
        for i, f in enumerate(funcs):
            x = cx_start + i*(cw + gap)
            self._circle(s, x, 5.2, cw, cw, C['kawaru'])
            self._block(s, x, 5.2, cw, cw, f, 14, bold=True,
                        color=C['white'],
                        align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)

        self._block(s, 0.5, 6.95, 12.33, 0.4,
                    '月35万円×5人 = 月175万円分の業務をAI社員にやってもらう',
                    13, color=C['gray'], align=PP_ALIGN.CENTER, bold=True)

        self._notes(s, """【テーマ・15秒】
今回のハッカソンで私が取り組むテーマです。
9月末までに、AI社員5人を作る。
マーケ、営業、デリバリー、事務法務、戦略の5機能を担うAI社員を、9月末までに完成させます。
