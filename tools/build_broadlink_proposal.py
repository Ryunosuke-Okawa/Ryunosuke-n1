...
class BroadlinkBuilder(ProposalBuilder):
    ...
    def concept_slide(self):
        ...
        sy = y + 1.65
        story = [
            ('ゲーム性', '研修にゲーム性を取り入れ、参加者が能動的に手と頭を動かす設計にする'),
            ('体感→腹落ち', '座学ではなく"体感"を起点にすることで、概念がそのまま身体に染み込む'),
            ('実戦性', 'ハンズオン50分で、研修終了後すぐに業務で使える状態まで持っていく'),
            ('飽きさせない', '8年目のISMS研修だからこそ、毎回違う"驚き"を仕込んだ構成にする'),
        ]
        n = len(story)
        avail = 7.30 - sy
        rh = (avail - 0.10 * (n - 1)) / n
...
def main():
    ...
    # 10. 弊社方針（金額は専用スライドで扱うのでここでは触れない）
    b.approach_slide([
