...
LP内で `assets/n1-logo.jpg` のように画像を呼び出している場合、画像ファイルも一緒にアップロードする必要があります。

1. `~/Desktop/lp_deploy/` の中に `assets/` フォルダを作る
2. その中に `n1-logo.jpg` などの素材を入れる
3. ローカルのフォルダ構成は最終的にこうなる：
...
> [!warning] ファイル名は変えない
> HTMLの中で `assets/n1-logo.jpg` のように名前で画像を呼び出しています。ファイル名を変えるとリンクが切れて画像が表示されなくなります。**中身だけ差し替え、名前は据え置き**が原則。
...
- [ ] 画像ファイル名のスペル一致（HTMLの呼び出しと完全一致しているか）
- [ ] `assets/` フォルダの場所（サブドメイン直下にあるか）
- [ ] 画像の拡張子（`.jpg` と `.JPG` は別物として扱われる）
